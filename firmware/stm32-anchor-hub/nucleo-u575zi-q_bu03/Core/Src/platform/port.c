...
SPI_HandleTypeDef *hcurrent_active_spi = &hspi2;
uint16_t          pin_io_active_spi = UWB_CS_Pin;
GPIO_PinState     SPI_CS_state = GPIO_PIN_RESET;
host_using_spi_e host_spi = SPI_2;
...
void reset_DWIC(void)
{
    ...
    // Enable GPIO used for DW3xxx/QM33xx reset as open collector output
...
/* @fn         UWB_GPIO_EXTI_Callback
 * @brief      EXTI line detection callback for UWB
 * @param      GPIO_Pin: Specifies the port pin connected to corresponding EXTI line.
 *             i.e. UWB_RST_Pin and UWB_IRQ_Pin
 */
void UWB_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
    switch (GPIO_Pin)
    {
        //    case DW_RST_B_Pin :
    ...
    case UWB_IRQ_Pin :
        {
            //while (HAL_GPIO_ReadPin(UWB_IRQ_GPIO_Port, UWB_IRQ_Pin) == GPIO_PIN_SET)
            {
                process_deca_irq();
                //dwt_isr();
            }
...
__INLINE void process_deca_irq(void)
{
    while (port_CheckEXT_IRQ() != 0)
    {
        ...
        {
            break; /* Prevent infinite loop if ISR is not registered yet! */
        }
...
__INLINE uint32_t port_CheckEXT_IRQ(void)
{
    ...
    if(SPI_1 == host_spi)
    {
        status = HAL_GPIO_ReadPin(UWB_IRQ_GPIO_Port, UWB_IRQ_Pin);;
    }
    else
    {
        status = HAL_GPIO_ReadPin(UWB_IRQ_GPIO_Port, UWB_IRQ_Pin);
