...
int32_t writetospiwithcrc(uint16_t headerLength, const uint8_t *headerBuffer, uint16_t bodyLength, const uint8_t *bodyBuffer, uint8_t crc8)
{
    ...
    HAL_GPIO_WritePin(UWB_CS_GPIO_Port, pin_io_active_spi, SPI_CS_state); /**< Put chip select line low */
    ...
    HAL_GPIO_WritePin(UWB_CS_GPIO_Port, pin_io_active_spi,(GPIO_PinState)(!((uint8_t)SPI_CS_state))); /**< Put chip select line high */
...
int32_t readfromspi(uint16_t headerLength, uint8_t *headerBuffer, uint16_t readlength, uint8_t *readBuffer)
{
    ...
    uint16_t total_len = headerLength + readlength;

    if (total_len <= 128) {
        memcpy(tx_buf, headerBuffer, headerLength);
        HAL_SPI_TransmitReceive(hcurrent_active_spi, tx_buf, rx_buf, total_len, HAL_MAX_DELAY);
        memcpy(readBuffer, &rx_buf[headerLength], readlength);
    } else {
        /* Fallback for long reads (if any) */
        HAL_SPI_Transmit(hcurrent_active_spi, (uint8_t*)headerBuffer, headerLength, HAL_MAX_DELAY);
        HAL_SPI_Receive(hcurrent_active_spi, readBuffer, readlength, HAL_MAX_DELAY);
    }
